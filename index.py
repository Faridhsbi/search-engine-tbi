...
class InvertedIndex:
    ...
    avg_doc_length: float
        Average document length across the collection. Used for BM25 scoring.

    max_tf: dict
        key: term ID (int), value: maximum TF of that term across all documents.
    ...
    def __init__(self, index_name, postings_encoding, directory=''):
        ...
        self.avg_doc_length = 0.0   # Average document length for BM25
        self.max_tf = {}        # key: term ID (int), value: max TF for that term (for WAND)
    ...
    def __enter__(self):
        ...
        Metadata:
            ...
            4. avg_doc_length, rata-rata panjang dokumen di koleksi. Untuk BM25.
            5. max_tf, dictionary dari term_id ke max TF. Untuk WAND.
        ...
        with open(self.metadata_file_path, 'rb') as f:
            data = pickle.load(f)
            # Support both old format (3-tuple) and new format (5-tuple)
            if len(data) == 3:
                self.postings_dict, self.terms, self.doc_length = data
                self.avg_doc_length = (sum(self.doc_length.values()) / len(self.doc_length)) if self.doc_length else 0.0
                self.max_tf = {}
            elif len(data) == 5:
                self.postings_dict, self.terms, self.doc_length, self.avg_doc_length, self.max_tf = data
            else:
                self.postings_dict, self.terms, self.doc_length = data[0], data[1], data[2]
    ...
    def __exit__(self, exception_type, exception_value, traceback):
        ...
        # Menyimpan metadata (postings dict, terms, doc_length, avg_doc_length, max_tf)
        with open(self.metadata_file_path, 'wb') as f:
            pickle.dump([self.postings_dict, self.terms, self.doc_length,
                         self.avg_doc_length, self.max_tf], f)
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        3. Menyimpan metadata dalam bentuk self.terms, self.postings_dict,
           self.doc_length, dan self.max_tf.
        ...
        # update self.max_tf for WAND upper bounds
        max_tf_val = max(tf_list) if tf_list else 0
        if term not in self.max_tf:
            self.max_tf[term] = max_tf_val
        else:
            self.max_tf[term] = max(self.max_tf[term], max_tf_val)
    ...
    def compute_avg_doc_length(self):
        """
        Compute the average document length across the collection.
        Should be called after all terms have been appended.
        """
        if self.doc_length:
...
if __name__ == "__main__":
    ...
    with InvertedIndexWriter('test', postings_encoding=VBEPostings, directory='./tmp/') as index:
        ...
        assert index.max_tf == {1: 30, 2: 56}, "max_tf salah"
        ...
        index.compute_avg_doc_length()
        assert abs(index.avg_doc_length - (2+38+25+56+3+30)/6) < 0.001, "avg_doc_length salah"
